...
temp = np.zeros((3, len(cat)))
for v in np.unique(cat['visit']):
    temp[:,cat['visit']==v] = (
        summary[summary['visit']==v]['rawseeing'],
        summary[summary['visit']==v]['airmass'],
        summary[summary['visit']==v]['skybg'],
    )
...
for ax, param, label, ticks in zip(
    a.flatten(),
    ['N', 0, 1, 2],
    [r'$N$', 'seeing', 'airmass', 'sky background'],
    ...
    if param=='N':
        sp.draw_hpxbin(
            cat['ra'], cat['dec'],
            zoom=True, xsize=600, cmap=cc.cm['CET_L19'])
    else:
        sp.draw_hpxbin(
